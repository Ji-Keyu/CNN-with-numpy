...
def affine_forward(x, w, b):
    ...
    out = np.matmul(x.reshape(x.shape[0], -1), w) + b[np.newaxis, :]
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
      ...
      x_tilt = (x-sample_mean[np.newaxis,:])/(np.sqrt(sample_var[np.newaxis,:]+eps))
      out = gamma[np.newaxis,:]*x_tilt + beta[np.newaxis,:]
      cache = (sample_mean, sample_var, gamma, beta, x, x_tilt, eps)
      ...
      x = (x-running_mean[np.newaxis,:])/(np.sqrt(running_var[np.newaxis,:]+eps))
      out = gamma[np.newaxis,:]*x + beta[np.newaxis,:]
        # =====================================================
...
def batchnorm_backward(dout, cache):
    ...
    # ================================================================ #
    N, D = dout.shape
    mean, var, gamma, beta, x, x_tilt, eps = cache
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout*x_tilt, axis=0)
    ### the rest is all for dx
    dx_tilt = dout*gamma[np.newaxis,:]
    da = dx_tilt/(np.sqrt(var[np.newaxis,:]+eps))
    dmean = -np.sum(dx_tilt, axis=0)/(np.sqrt(var+eps)) 
    dvar = -0.5*np.sum((x-mean[np.newaxis,:])*dx_tilt/(var[np.newaxis,:]+eps)**1.5, axis=0)
    dmean -= dvar*2/N*np.sum(x-mean[np.newaxis,:], axis=0)
    dx = da + 2/N*(x-mean[np.newaxis,:])*dvar+dmean/N
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
